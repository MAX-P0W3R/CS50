...
def main():
    ...
    # TODONE: Check for command-line usage
    if len(sys.argv) != 3:
        sys.exit("Usage: python dna.py databases/FILENAME.csv sequences/FILENAME.txt")

    # TODONE: Read database file into a variable
    database = []
    ...
    # TODONE: Read DNA sequence file into a variable
    txtfile = open(sys.argv[2],"r")
    dna = txtfile.read()

    # TODONE: Find longest match of each STR in DNA sequence
    strs_dna = {}
    for subsequence in subsequences:
        longest_run = longest_match(dna, subsequence)
        strs_dna[subsequence] = longest_run

    # TODONE: Check database for matching profiles
